 ...
 * once every two seconds. If put together with other fireflies, it causes complex patterns to emerge. 
 * the hardware uses a light sensor (a photo transistor) to detect flashes from other fireflies
 ...
 * default sketch file to the device to wipe the chip clean of the original code. (has to be done within 8 seconds on Arduino Pro-Micro)
 * 2015 Muon Ray Enterprises
 * 
 * Chip type: ATmega32U4
 * Clock frequency: Default internal clock 16MHz 
 * So clock can be set as 16MHz/4 = 4MHz max for each pin. 
 * 
 * General Pinout Diagram
 *                       +--------+
 *        [        (GND) |        | (VCC)  Power ]
 *        [              |        |
 *        [              |      15| (PB1)   BLUE ]
 *        [              |      14| (PB3)  GREEN ]
 *        [PhotoDet(PB4) |8     16| (PB2)    RED ]
 *        [        (PB5) |9     10| (PB6)        ]
 *                       +--------+
 */
...
#define FLASH_DELAY 1000       // how long lasts the flash
#define DAYLIGHT 200          // values higher than that are recognized as daylight (for CdS 160 , for phototransistor 240) 
#define DAYLIGHT_DELAY 5000  // wait 10 seconds, if daylight detected
#define BLIND_AFTER_OTHER 500 // how long are we blind after another flash
 ...
 * 
 * Normally the HSV model needs a full circle (360ï¿½) to distribute
 * all rainbow colors over this circle. But 360 is too big for a
 * single byte. So we use a small trick here. We DEFINE simply,
 * that our circle has only 252ï¿½, not 360ï¿½. Also 252 is dividable 
 ...
 * hue from 0ï¿½ to 360ï¿½. Hue(252) stands for the hue, that we are
 * using, from 0ï¿½ to 252ï¿½.
 *
...
int main(void) {  
  ...
  g = 0;                     // switch color to green        // can optionally switch off here for night light function
