 ...
 * light sensor (a photo transistor or ldr) to detect flashes from other fireflies
 * nearby. The program tries to synchronize its own flash with the flash of
 * its neighbours.
 * 
 *
 * Default Design for RGB-LED commond cathode type.
 * with hIGH it ligths up the led and with Low it lights off the LED
 * 
 * With a Common Anode LED it high lights off because its INVERTED VOLTAGE
 * 
 *Simplest Way is to rearrange HI/LOW definitions
 *For Common Cathode LED Switches
 *#define HIGH 1
 *#define LOW 0
 *
 *For Common Anode LED Switches
 *
 *#define HIGH 0
 *#define LOW 1
 *
...
SIGNAL(TIM0_OVF_vect) {	
  ...
  if (++softscale == 0) {    
    ...
    // Experiment:
    // change here for common anode rgb led?
    // off value for led changes from 0 to 1 for common anode, 1 to 0 for common cathode...perhaps?
...
int main(void) {	
  ...
  // enable pins as output (change for common anode RGD LED)
  DDRB |= 
